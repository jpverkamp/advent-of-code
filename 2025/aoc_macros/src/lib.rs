...
pub fn test(input: TokenStream) -> TokenStream {
    ...
    impl Parse for TestInput {
        fn parse(input: ParseStream) -> syn::Result<Self> {
            let day = input.parse()?;
            input.parse::<Token![,]>()?;
            let input_path: LitStr = input.parse()?;
            ...
            Ok(TestInput { day, input_path, cases })
        }
    }
    
    let test_input = parse_macro_input!(input as TestInput);
    let day_str = expr_to_string(&test_input.day);
    let input_path = test_input.input_path.value();
    
    // Generate test functions for all cases
    let test_functions: Vec<_> = test_input.cases.iter().flat_map(|test_case| {
        ...
        test_case.solutions.iter().map(move |solution_expr| {
            ...
            let test_name = format_ident!("test_{}_{}", day_str, name_str);
            let name_lit = name_str.clone();
            let input_path_lit = input_path.clone();
            ...
            quote! {
                #[test]
                fn #test_name() {
                    let input = std::fs::read_to_string(#input_path_lit)
                        .unwrap_or_else(|e| panic!("failed to read {}: {}", #input_path_lit, e));
